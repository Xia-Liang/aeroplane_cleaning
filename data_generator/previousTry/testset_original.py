...
generate testset of lidar and semlidar sensor
...
def draw_image(surface, image):
    ...
    if image.frame % 7 == 0:
        image.save_to_disk('D:\\mb95541\\aeroplane\\data\\testset\\original\\rgb\\%d' % image.frame)
...
def save_lidar(lidar):
    ...
    if lidar.frame % 7 == 0:
        ...
        np.save('D:\\mb95541\\aeroplane\\data\\testset\\original\\lidar\\%d' % get_name(lidar.frame), data)


def save_sem_lidar(lidar_sem):
    if lidar_sem.frame % 7 == 0:
        ...
        np.save('D:\\mb95541\\aeroplane\\data\\testset\\original\\semLidar\\%d' % get_name(lidar_sem.frame), data)
