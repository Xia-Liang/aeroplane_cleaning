...
def draw_image(surface, image):
    ...
    if image.frame % 17 == 0:
        image.save_to_disk('D:\\mb95541\\aeroplane\\data\\testset\\rgb\\%d' % image.frame)
...
def save_lidar(lidar):
    ...
    if lidar.frame % 17 == 0:
        ...
        np.save('D:\\mb95541\\aeroplane\\data\\testset\\lidar\\%d' % get_name(lidar.frame), data)
...
def save_sem_lidar(lidar_sem):
    if lidar_sem.frame % 17 == 0:
        ...
        np.save('D:\\mb95541\\aeroplane\\data\\testset\\semLidar\\%d' % get_name(lidar_sem.frame), data)
