...
print how many points in each tag (prediction and real)
...
def main():
    ...
    classifier.load_state_dict(torch.load('model\\seg_model_449_best.pth'))
    classifier.eval()
        ...
        with CarlaSyncMode(world, rgb_camera, lidar, lidar_sem, fps=30) as sync_mode:
            while True:
                ...
                if lidar.frame > 200 & lidar.frame % 29 == 0:
                    ...
                    if lidar_array.shape[0] < 4500:
                        continue

                    # drop redundant point, choose only 4500 points
