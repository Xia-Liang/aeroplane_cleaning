...
def save_sem_lidar(lidar_sem, cases):
    if lidar_sem.frame % 13 == 0:
        ...
        global_location = np.concatenate((data_float[:, 1:2], -1 * data_float[:, 0:1], data_float[:, 2:]), axis=1)
        global_location = global_location + [global_x, global_y, global_z] - [280, 270, 0]  # airplane location [280 270 0.055]

        data = np.concatenate((global_location, data_int), axis=1)  # xyz, tag

        print(data.shape, end=' ')
        print('location', global_x, global_y, global_z)
